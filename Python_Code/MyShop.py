...
def get_product_Details():
    ...
    for i in range(count):
        ...
        print()
        isgiftwrap = int(input("Do we have to wrap it as gift please enter 1 for yes and 0 for No : ").strip())
        print()
...
def discountCalculator(details, Original_product):
    ...
    discount_rule ={
        ...
        "tiered_50_discount":total_quantity > 30 and any(quantity > 15 for quantity in individu_quanity.values())
    } 
    ...
    applicable_discounts = [rule for rule, apply_condition in discount_rule.items() if apply_condition]
    ...
    discount_amount = {"flat_10_discount": max(flat_10_discount_list), "bulk_5_discount":max(bulk_5_discount_list), "bulk_10_discount":max(bulk_10_discount_list),"tiered_50_discount":max(tiered_50_discount_list)} 
    max_discount = 0
    applicable_rule = ""
...
#  here we are printing the Oder Summery 
def ShowSummery(details, discount, fee):
...
if __name__ == '__main__':
    ...
    detailList = get_product_Details()
    
    details = calculateDetails(detailList, product)

    max_discount = discountCalculator(details, product) 
    
    fee = calculate_page_and_giftrap_charge(details["total_quantity"], details["individual_Product_bool"], details["individual_Product_quantity"])

    ShowSummery(details, max_discount, fee) 
